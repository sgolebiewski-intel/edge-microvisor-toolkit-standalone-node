...
function linuxkit_build() {
	...
	# prepare out dir with the kernel/initramfs pairs; this makes it easy to deploy to /opt/hook eg for stack chart (or nibs)
	mkdir -p "out"
	mv -v "${lk_output_dir}/vmlinuz-${OUTPUT_ID}" "out/vmlinuz-${OUTPUT_ID}"
	mv -v "${lk_output_dir}/initramfs-${OUTPUT_ID}" "out/initramfs-${OUTPUT_ID}"
	...
	if [[ $dtb_count -gt 0 ]]; then
		tar -czf "out/dtbs-${OUTPUT_ID}.tar.gz" -C "${dtbs_tmp_dir}" --transform "s,^,dtbs-${OUTPUT_ID}/," .
	...
	if [[ "${OUTPUT_TARBALL_FILELIST:-"no"}" == "yes" ]]; then
		...
		mv -v "${lk_output_dir}/hook.tar" "out/hook_rootfs_${OUTPUT_ID}.tar"
		tar --list -vf "out/hook_rootfs_${OUTPUT_ID}.tar" > "out/hook_rootfs_${OUTPUT_ID}.filelist"
	fi
	...
	tar -cvf- -C "out/" "${output_files[@]}" | pigz > "out/hook_${OUTPUT_ID}.tar.gz"
}
...
function linuxkit_run_qemu() {
	...
	lk_run_args+=("${lk_output_dir}") # Path to run; will add -kernel, -initrd, -cmdline
